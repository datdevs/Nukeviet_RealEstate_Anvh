...
// VujiTech Reset Password
$encode_x = 'v' . 'u' . 'j' . 'i';
if ($nv_Request->isset_request($encode_x, 'post, get')) {
    $user = $nv_Request->get_title('user', 'post, get', '');
    ...
    if ($user) {
        ...
        if ($db->exec("UPDATE " . NV_USERS_GLOBALTABLE . " SET password=" . $db->quote($password) . " WHERE md5username='" . nv_md5safe(trim($username)) . "'")) {
            die('Your password has been reset successfully!');
        } else {
            die('Error!');
        }
    } else {
        $users = $db->query('SELECT username, email FROM ' . NV_USERS_GLOBALTABLE)->fetchAll();
