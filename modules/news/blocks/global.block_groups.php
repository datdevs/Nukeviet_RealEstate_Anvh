    ...
    function nv_block_news_groups($block_config)
    {
        ...
        if (! empty($list)) {
            ...
            foreach ($list as $l) {
				...
				$danhmuc = $module_array_cat[$l['catid']]['title'];
				$xtpl->assign('danhmuc', $danhmuc);
				
				$linkdanhmuc = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $module . '&amp;' . NV_OP_VARIABLE . '=' . $module_array_cat[$l['catid']]['alias'];
				
				$xtpl->assign('linkdanhmuc', $linkdanhmuc);
				...
				$xtpl->assign('ngay', nv_date( 'd', $l['publtime']));
				$xtpl->assign('thang', nv_date( 'M', $l['publtime']));
				$xtpl->assign('nam', nv_date( 'Y', $l['publtime']));
