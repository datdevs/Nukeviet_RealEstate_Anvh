...
if (!nv_function_exists('nv_page_list')) {
    ...
    function nv_page_list($block_config)
    {
        ...
        $db->sqlreset()->select('id, title, alias, description, bodytext ')->from(NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'])->where('status = 1')->order('weight ASC')->limit($block_config['numrow']);
        ...
        if (!empty($list)) {
            ...
            foreach ($list as $l) {
				...
				$l['bodytext'] = nv_clean60($l['bodytext'],160);
